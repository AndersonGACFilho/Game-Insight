...
// GameLanguageSupport indicates localized support type for a specific language.
// language_code & support_type_code store numeric codes referencing enumeration_lookup.
// Convenience decoded fields (LanguageISO, SupportType) are ignored by GORM.
type GameLanguageSupport struct {
	...
	// Decoded convenience fields (populate after query if needed)
